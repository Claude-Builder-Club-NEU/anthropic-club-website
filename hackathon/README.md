...
| **Every flood dot is 2 × 2, never 1 × 1.** | Filter primitive subregions are in user space and get scaled by the device pixel ratio before rasterising, so below 1× — which is any browser zoom under 100% — a 1px dot rounds away, the tile grid comes back empty and the filtered text renders **nothing at all**. The wordmark vanished, and that is how this was caught. Each filter's dilation radius is dropped by exactly what its dot gains, so the block size is unchanged: `#px` is a 2 × 2 dot and radius 3 rather than 1 × 1 and radius 3.5, which is 8px blocks either way. **Do not "simplify" these back to 1 × 1.** |
...
| **A Seats section, after Tracks.** | The figure sits behind the meter at poster size in Funnel Display, pixelated through the same `#hk-pixel` as the hero wordmark, so the two big numerals on the page are the same object. The meter is **100 characters wide, one column per seat**: capacity is 100, so the bar is a literal count, not a proportion. Three rows, offset along the ramp, with **no background and no border**, so the number shows through the gaps between glyphs and the ribbon runs *through* it rather than sitting on it in a black box — built the same way as the ribbon above the sponsor tiers. `68% FULL` sits at the right-hand end, where the free seats are dim dots and a small pad costs nothing. |
...
| **The bar cuts the figure in half.** | `.hk-seats__figure` is masked with a channel knocked straight through the numerals, and the bar is sized and positioned to fill it exactly — same centre line, same height, both derived from `--hk-meter-half` so they cannot drift. That is also what earns the rectangle: a slab lying across the numerals would bury them; set into them it does not. The centre line is 54.6%, not 50%, because with `line-height: 0.82` the digits' ink centre is not the box centre. The figure is `clamp(150px, 23vw, 420px)` — bigger than the wordmark's 13.9vw, because the bar takes a slice out of the middle of it and the halves that survive still have to read. |
...
- [ ] **The wordmark face.** Funnel Display 800 through `#hk-pixel`, which is
      the answer to "normal text with pixelated edges" rather than a face the
      user picked. Two specimen sheets of alternatives were rejected. Not
      blocking; only open in the sense that nobody has said yes to it.
- [ ] **Details copy.** Meals, quiet room, team size, student ID and "Free for
      students" in `src/lib/details.js`.
- [ ] **Sponsors.** `src/lib/sponsors.js` — replace each `null` with
      `{ src, alt, height }`. Monochrome white, 28–32px tall.
...
hero wordmark resolves `url(#hk-pixel)` on Funnel Display.
